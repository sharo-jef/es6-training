...
function isLessonUnlocked(chapterIndex, lessonIndex) {
    ...
    const { prevChapter, prevLesson } = getPreviousLessonIndices(chapterIndex, lessonIndex);
...
function getPreviousLessonIndices(chapterIndex, lessonIndex) {
    ...
    if (chapterIndex > 0) {
        ...
        return { prevChapter: chapterIndex - 1, prevLesson: prevChapter.lessons.length - 1 };
    }
...
function render() {
    ...
    if (isExerciseView && lesson.exercise) {
        ...
        if (monacoEditor && monacoEditor.getValue().trim() === '' && lesson.exercise.starterCode) {
        ...
        if (slide) {
            slideTitleEl.textContent = `${lesson.title} (${currentSlideIndex + 1}/${lesson.slides.length})`;
...
function updateChapterList() {
    ...
    courseData.forEach((chapter, chapterIdx) => {
        ...
        chapterHeader.className = 'text-lg font-bold text-gray-800 dark:text-gray-200 px-3 pt-4 pb-2';
        ...
        chapter.lessons.forEach((lesson, lessonIdx) => {
            ...
            button.innerHTML = `
                ...
                ${isCompleted ? '<svg class="w-5 h-5 text-green-500" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>' : ''}
            `;
            button.className = 'w-full text-left p-3 rounded-md transition duration-200 flex items-center justify-between';
            ...
            if (isCurrent) {
                ...
                button.classList.add('text-gray-400', 'dark:text-gray-500', 'cursor-not-allowed');
...
function updateNavigation() {
    ...
    prevBtn.disabled = (currentChapterIndex === 0 && currentLessonIndex === 0 && currentSlideIndex === 0);
    ...
    const { nextChapter, nextLesson } = getNextLessonIndices(currentChapterIndex, currentLessonIndex);
    ...
    if (isExerciseView) {
        if (isLastLesson) {
            ...
            nextBtn.disabled = !isLessonCompleted(currentChapterIndex, currentLessonIndex);
    ...
    if (!isExerciseView && lesson.slides.length > 0) {
        slideCounterEl.textContent = `${currentSlideIndex + 1} / ${lesson.slides.length}`;
        slideCounterEl.classList.remove('hidden');
    } else {
        slideCounterEl.classList.add('hidden');
    }

    checkBtn.disabled = !isExerciseView || !lesson.exercise || isLessonCompleted(currentChapterIndex, currentLessonIndex);
}
...
function updateProgressBar() {
    ...
    courseData.forEach((chapter, chapterIdx) => {
        chapter.lessons.forEach((lesson, lessonIdx) => {
            if (lesson.exercise) { // Only count lessons with exercises towards progress
                totalLessons++;
...
function handleNext() {
    const lesson = getLesson(currentChapterIndex, currentLessonIndex);
    const isLastLesson = getNextLessonIndices(currentChapterIndex, currentLessonIndex).nextChapter === null;

    if (isLastLesson && isLessonCompleted(currentChapterIndex, currentLessonIndex)) {
        showCompletionModal();
        return;
    }
    ...
    if (currentSlideIndex < lesson.slides.length - 1) {
        ...
        if (isLessonCompleted(currentChapterIndex, currentLessonIndex)) {
            ...
            if (nextChapter !== null) {
                ...
                currentLessonIndex = nextLesson;
                currentSlideIndex = 0;
                if (monacoEditor) monacoEditor.setValue(getLesson(currentChapterIndex, currentLessonIndex)?.exercise?.starterCode || '');
            }
        }
    }
    render();
...
function handlePrev() {
    ...
    if (currentSlideIndex > 0) {
        ...
        const { prevChapter, prevLesson } = getPreviousLessonIndices(currentChapterIndex, currentLessonIndex);
